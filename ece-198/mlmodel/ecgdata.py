...
class ECGData:
  ...
  def split_data(self, train_test_split = 0.3):
    test_len = math.floor(len(self.j_x_batch) * train_test_split)
    train_len = math.floor(len(self.j_x_batch) - test_len)

    np.random.shuffle(self.j_x_batch)
    np.random.shuffle(self.j_y_batch)
